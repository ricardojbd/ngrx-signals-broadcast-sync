...
test.describe('Channel Options Synchronization Test', () => {
  const pageUri = '/channelOptions';
  ...
  test('should display broadcast channel name', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="channel-value"]')).toHaveText('channelOptions');
    await expect(page2.locator('[data-testid="channel-value"]')).toHaveText('channelOptions');
